...
\begin{proposition}{Compuesto de dos extensiones admisibles}{}
...
\]
es el menor subcuerpo de \(L\) que contiene a \(L_{1}\) y \(L_{2}\).
\end{proposition}
...
\begin{proposition}{Subanillo y subcuerpo generados}{}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \(R\) es cerrado bajo suma y producto: dados dos elementos, juntamos los conjuntos finitos de \(s_i\) usados y los expresamos como polinomios en esas variables. La suma/producto de polinomios es un polinomio. 
        \item Luego \(R\) es un subanillo que contiene \(K\) y \(S\). 
        \item Si \(R'\) es otro subanillo con \(K \cup S \subseteq R'\), entonces \(R'\) contiene todos los polinomios en elementos de \(S\), luego \(R \subseteq R'\). 
    \end{itemize}
    Por tanto, \(R = K[S]\).

    \item[(b)] Sea \(F = \{ p(s_1,\dots,s_n)/q(s_1,\dots,s_n) \mid \dots \}\). 
    \begin{itemize}
        \item \(F\) es un subcuerpo: suma, producto e inversos se reducen a operaciones con polinomios. 
        \item Claramente \(K \cup S \subseteq F\). 
